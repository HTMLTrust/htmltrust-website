/* HTMLTrust canonicalization browser bundle, pinned to @htmltrust/canonicalization v0.2.2. */
(() => {
 ...
 */
function extractCanonicalText(html, options = {}) {
 ...
 */
function extractClaimsFromSignedSection(html) {
  if (typeof html !== "string") {
 ...
 * @returns {string}
 */
function buildSignatureBinding({ contentHash, claimsHash, domain, signedAt }) {
...
function validateSerializedOrigin(origin) {
  let url;
  try {
    url = new URL(origin);
  } catch {
 ...
 */
async function verifySignature(message, signatureB64, publicKeyPem, algorithm = "ed25519") {
 ...
 * @returns {boolean}
 */
function isKeyRevoked(key, now = Date.now()) {
  if (!key) return false;
 ...
 * @returns {KeyResolver}
 */
function directUrlResolver(opts = {}) {
  return {
    async resolve(keyid) {
 ...
 * @returns {KeyResolver}
 */
function trustDirectoryResolver(opts) {
 ...
 */
async function resolveKey(keyid, resolvers) {
 ...
 * @returns {string}
 */
function buildEndorsementBinding(e) {
 ...
 * @returns {Promise<boolean>}
 */
async function verifyEndorsement(endorsement, resolvers) {
